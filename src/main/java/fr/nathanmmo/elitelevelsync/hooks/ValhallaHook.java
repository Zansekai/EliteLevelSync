...
public class ValhallaHook implements LevelHook {
    ...
    public ValhallaHook(String levelType) { this.levelType = levelType == null ? "LEVEL" : levelType.toUpperCase(); }
    @Override public String getName() { return "ValhallaMMO"; }
    @Override public int getLevel(Player player) {
        try {
            PowerProfile p = CharacterProfile.getOrCache(player, PowerProfile.class);
            if (p == null) return -1;
